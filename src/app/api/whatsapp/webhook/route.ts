...
import { normalizePhone, phonesMatch, sanitizePhoneForMeta } from '@/lib/whatsapp/phone-utils'
import { verifyMetaWebhookSignature } from '@/lib/whatsapp/webhook-signature'
import { runAutomationsForTrigger } from '@/lib/automations/engine'
import { dispatchInboundToFlows } from '@/lib/flows/engine'
import { handleCustomerOnboarding, sendMessageFromBot } from '@/lib/whatsapp/onboarding'
...
export async function GET(request: Request) {
  try {
    ...
    if (configError || !configs) {
      ...
      return NextResponse.json(
        { error: `Verification failed: ${configError?.message || 'Configs not found'}` },
        { status: 403 }
      )
    }
...
export async function POST(request: Request) {
  ...
  console.log('[webhook POST] Incoming Meta Webhook body:', rawBody)
  ...
  if (!verifyMetaWebhookSignature(rawBody, signature)) {
    ...
    console.warn('[webhook POST] Rejected request with invalid signature.')
...
async function processMessage(
  ...
  // ============================================================
  // Intercept for customer onboarding if not yet completed (Sri Vasavi Plywoods)
  // ============================================================
  const inboundText = contentText ?? message.text?.body ?? ''
  const onboardingHandled = await handleCustomerOnboarding(
    userId,
    contactRecord.id,
    conversation.id,
    inboundText,
  ...
  if (onboardingHandled) {
    // Onboarding chatbot handled the message. Skip flows, AI and automations.
    return
  }

  // ============================================================
  // Sri Vasavi Plywoods Gemini AI Auto-Reply System
  // ============================================================
  const cleanPhoneForAI = sanitizePhoneForMeta(contactRecord.phone)
  const { data: customerRecord } = await supabaseAdmin()
    .from('customers')
    .select('is_ai_enabled, whatsapp_onboarding_completed')
    .or(`phone.eq.${contactRecord.phone},phone.eq.${cleanPhoneForAI}`)
    .eq('is_deleted', false)
    .maybeSingle()

  if (customerRecord && customerRecord.is_ai_enabled) {
    // 1. Fetch recent conversation history (last 6 messages) for context
    ...
    // 2. Generate and trigger Gemini AI response
    const aiResponseText = await generateGeminiReply(
      userId,
      contactRecord.id,
      history,
      inboundText
    ...
    if (aiResponseText) {
      ...
      // Stop execution as Gemini successfully answered the query!
      return
    }
  }
